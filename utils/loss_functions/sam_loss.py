...
class Mask_DC_and_BCE_loss(nn.Module): # dice_weight=0.8
...
class preseg_Mask_DC_and_BCE_loss(nn.Module):
    def __init__(self, pos_weight, dice_weight=0.8):
        """
        DO NOT APPLY NONLINEARITY IN YOUR NETWORK!
        THIS LOSS IS INTENDED TO BE USED FOR BRATS REGIONS ONLY
        ...
        :param aggregate:
        """
        super(preseg_Mask_DC_and_BCE_loss, self).__init__()
    ...
    def forward(self, pred, target):
        if len(target.shape) == 5:
            ...
            pred = pred.view(-1, pred.shape[2], pred.shape[3], pred.shape[4])
...
def get_pre_seg_criterion(dice_weight=0.8):
    if torch.cuda.is_available():
        device = torch.device("cuda")
        pos_weight = torch.ones([1]).cuda(device=device) * 2
    else:
    ...
    criterion = preseg_Mask_DC_and_BCE_loss(pos_weight=pos_weight, dice_weight=dice_weight)
    return criterion