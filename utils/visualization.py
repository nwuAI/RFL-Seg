...
def visualize_predict_and_seg(seg, image_filename, save_path):
    save_directory = os.path.join(save_path, "fine_mask")

    os.makedirs(save_directory, exist_ok=True)

    for i in range(seg.shape[0]):

        base_name = os.path.basename(image_filename[i]).split('.')[0]


        seg_image = (seg[i, :, :] * 255).astype(np.uint8)
        seg_path = os.path.join(save_directory, f"{base_name}.png")
...
def visual_segmentation_sets_with_pt(seg, image_filename, opt, pt):
    ...
    fulldir = opt.result_path + "/mask_with_img/"
...
def visualize_preseg(seg, image_filename, save_path):
    save_directory = os.path.join(save_path, "coarse_mask")
